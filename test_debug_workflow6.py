...
async def main():
    ...
    for n in workflow['nodes']:
        ...
        print(f"  Node {nid}: type={ct}, widgets={wvs[:5] if wvs else []}")
    ...
    while removed:
        ...
        for nid in bad_ids:
            print(f"Removing node {nid} with bad link")
            prompt.pop(nid, None)
            removed = True
        if removed:
    ...
    print(f"\nAfter conversion: {len(prompt)} nodes")
    for nid in sorted(prompt.keys(), key=int):
        print(f"  Node {nid}: {prompt[nid].get('class_type')} - {prompt[nid].get('inputs')}")
