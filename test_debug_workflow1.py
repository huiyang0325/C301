...
async def main():
    ...
    # Print all nodes with id, type, widgets_values
    ...
    for n in workflow['nodes']:
        ...
        inputs = n.get('inputs', [])
        print(f"  Node {nid}: type={ct}, widgets={wvs[:6] if wvs else []}, inputs={[(i.get('name'), i.get('link')) for i in inputs[:4]]}")

    # Check node 6 specifically
    print("\nLooking for node 6...")
    for n in workflow['nodes']:
        if str(n.get('id')) == '6':
            print(f"  Node 6 found: type={n.get('type')}, widgets={n.get('widgets_values')}")
            print(f"  Node 6 inputs: {n.get('inputs')}")
    ...
    # Cascade removal
    print("\nPerforming cascade removal...")
    removed = True
    while removed:
        removed = False
        bad_ids = [nid for nid, d in prompt.items() if d.get('_bad_link')]
        if bad_ids:
            print(f"  Removing nodes with bad links: {bad_ids}")
        for nid in bad_ids:
            prompt.pop(nid, None)
            removed = True
        if removed:
                ...
                for key, val in list(d.get('inputs', {}).items()):
                    if isinstance(val, list) and len(val) == 2:
                        if str(val[0]) in bad_ids:
                            print(f"  Removing node {nid} - references {val[0]}")
                            prompt.pop(nid, None)
                            removed = True
                            break
    ...
    print("\nFinal prompt before submission:")
    for nid in sorted(prompt.keys(), key=int):
        d = prompt[nid]
        print(f"  Node {nid}: class={d.get('class_type')}, inputs={d.get('inputs')}")
