...
# Reroute must NOT be skipped - other nodes depend on them via links
SKIP_TYPES = {
...
def convert(workflow, available):
    ...
    # Reroute nodes should NOT be skipped - they are functional nodes other nodes depend on
    ...
    for node in workflow['nodes']:
        ...
        for inp in node.get('inputs', []):
            name = inp.get('name', '')
            # Reroute nodes have empty input name '' but API expects 'value'
...
async def main():
    ...
    for wf_name in workflows:
        ...
        try:
            ...
            # Cascade removal for nodes that reference skipped nodes
            removed = True
            while removed:
                removed = False
