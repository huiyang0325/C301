...
def convert_workflow(workflow, available, object_info=None):
    ...
    for node in workflow['nodes']:
        ...
        # Collect unlinked inputs in order (as they appear in node)
        unlinked_inputs = []
        ...
        # Build widget map: use sequential index for unlinked inputs
        input_widget_map = {}
        ...
        for key in list(inputs.keys()):
            ...
            if ct in ('KSampler', 'KSamplerAdvanced') and key == 'cfg':
                if not isinstance(val, (int, float)):
                    inputs[key] = 1.0
            if ct == 'KSamplerAdvanced' and key == 'return_with_leftover_noise':
