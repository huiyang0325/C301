"""调试工作流1 - 不跳过VRAMCleanup/RAMCleanup"""
import json, httpx, asyncio

WORKFLOW_DIR = r'E:\咸鱼买工作流\模型和插件，下载后把里面文件夹剪切到压缩包解压后得comfyui文件夹内覆盖即可(1)\ComfyUI-aki-v2\ComfyUI\user\default\workflows'

# VRAMCleanup and RAMCleanup are memory management nodes that should NOT cause cascade removal
# Other nodes can still function even if they pass data through cleanup nodes
SKIP_TYPES = {
...
def convert_workflow(workflow, available, object_info=None):
    ...
    # VRAMCleanup and RAMCleanup should NOT be in skip_ids - they are just utility nodes
    # that don't affect the main workflow execution
