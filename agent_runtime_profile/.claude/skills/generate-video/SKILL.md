...
```bash
# 标准模式：生成整集所有待处理场景（推荐）
cd projects/{project_name} && python ../../.claude/skills/generate-video/scripts/generate_video.py episode_{N}.json --episode {N}

# 断点续传：从上次中断处继续
cd projects/{project_name} && python ../../.claude/skills/generate-video/scripts/generate_video.py episode_{N}.json --episode {N} --resume

# 单场景：测试或重新生成
cd projects/{project_name} && python ../../.claude/skills/generate-video/scripts/generate_video.py episode_{N}.json --scene E1S1

# 批量自选：指定多个场景
cd projects/{project_name} && python ../../.claude/skills/generate-video/scripts/generate_video.py episode_{N}.json --scenes E1S01,E1S05,E1S10
