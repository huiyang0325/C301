...
| 阶段 | 触发条件 | 脚本命令 |
|------|---------|---------|
| 5. 角色设计 | 有角色缺少 character_sheet | 对每个缺失角色执行：`cd projects/{name} && python ../../.claude/skills/generate-characters/scripts/generate_character.py --character "{角色名}"` |
...
```
任务：为项目 {project_name} 生成缺失的角色设计图。
