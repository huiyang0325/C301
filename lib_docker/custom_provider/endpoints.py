...
def _build_minimax_chat(provider, model_id: str) -> CustomTextBackend:
    base_url = (provider.base_url or "https://api.minimaxi.com").rstrip("/")
    delegate = MiniMaxTextBackend(api_key=provider.api_key, base_url=base_url, model=model_id)
    return CustomTextBackend(provider_id=provider.provider_id, delegate=delegate, model=model_id)
...
ENDPOINT_REGISTRY: dict[str, EndpointSpec] = {
    ...
    "minimax-chat": EndpointSpec(
        key="minimax-chat",
        media_type="text",
        family="minimax",
        display_name_key="endpoint_minimax_chat_display",
        request_method="POST",
        request_path_template="/v1/chat/completions",
        build_backend=_build_minimax_chat,
    ),
