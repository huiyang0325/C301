...
e. **image_prompt**：生成包含以下字段的对象：
   - scene：用中文描述此刻画面中的具体场景——角色位置、姿态、表情、**服装颜色和样式等外观细节**，以及可见的环境元素和物品。
     **重要**：必须包含角色的具体外观描述（如"红发女子穿蓝色长裙"），而不仅仅是"女子"。
     外观描述必须与 characters_in_segment 中列出的角色定义一致，保持视觉一致性。
...
f. **video_prompt**：生成包含以下字段的对象：
   - action：**必须包含角色外观描述**。用中文精确描述该时长内主体的具体动作——身体移动、手势变化、表情转换。
     在描述动作前，先用一句话描述角色的外观特征（发型、服装颜色等），格式如："[红发女子，穿蓝色长裙] 走向窗边"或"她站起身"。
     外观描述必须引用 characters_in_segment 中列出的角色定义，保持视觉一致性。
...
d. **image_prompt**：生成包含以下字段的对象：
   - scene：用中文描述此刻画面中的具体场景——角色位置、姿态、表情、**服装颜色和样式等外观细节**，以及可见的环境元素和物品。{_format_aspect_ratio_desc(aspect_ratio)}。
     **重要**：必须包含角色的具体外观描述（如"红发女子穿蓝色长裙"），而不仅仅是"女子"。
     外观描述必须与 characters_in_scene 中列出的角色定义一致，保持视觉一致性。
     聚焦当下瞬间的可见画面。仅描述摄像机能够捕捉到的具体视觉元素。
     确保描述避免超出此刻画面的元素。排除比喻、隐喻、抽象情绪词、主观评价、多场景切换等无法直接渲染的描述。
     画面应自包含，不暗示过去事件或未来发展。
   - composition：
...
e. **video_prompt**：生成包含以下字段的对象：
   - action：**必须包含角色外观描述**。用中文精确描述该时长内主体的具体动作——身体移动、手势变化、表情转换。
     在描述动作前，先用一句话描述角色的外观特征（发型、服装颜色等），格式如："[红发女子，穿蓝色长裙] 走向窗边"或"她站起身"。
     外观描述必须引用 characters_in_scene 中列出的角色定义，保持视觉一致性。
     聚焦单一连贯动作，确保在指定时长内可完成。
     排除多场景切换、蒙太奇、快速剪辑等单次生成无法实现的效果。
     排除比喻性动作描述（如"像蝴蝶般飞舞"）。
   - camera_motion：镜头运动（Static, Pan Left, Pan Right, Tilt Up, Tilt Down, Zoom In, Zoom Out, Tracking Shot）
