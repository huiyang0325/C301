...
def convert_workflow(workflow, available, object_info=None):
    """转换工作流，使用object_info正确分配widget值"""
    link_map = {}
    ...
    for node in workflow['nodes']:
        ...
        # Get node's input order from object_info for correct widget mapping
        node_input_order = []
        ...
        # Build map: input name -> widget index (only for unlinked inputs)
        input_widget_map = {}
        if node_input_order and wvs:
            ...
            for inp in node.get('inputs', []):
                ...
                if link is not None and link in link_map:
                    continue  # Skip linked inputs - don't consume widget
                if wvs_idx < len(wvs):
