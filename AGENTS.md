...
frontend/ (React SPA)  →  server/ (FastAPI)  →  lib/ (核心库)
  ...
  zustand 状态管理          (Codex Agent SDK)     ProjectManager
```
...
- `assistant.py` — Codex Agent SDK 会话管理（SSE 流式）
...
### Agent Runtime（Codex Agent SDK 集成）

`server/agent_runtime/` 封装 Codex Agent SDK：
- `AssistantService` (`service.py`) — 编排 Codex SDK 会话
