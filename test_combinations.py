...
async def main():
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        r = await c.post('http://127.0.0.1:8188/prompt', json={'prompt': p1})
    print(f'  Status: {r.status_code}')

    # Test 2: With node 11 but different order
    print("\nTest 2: With node 11 at the end")
    p2 = {
        '1': {'class_type': 'UNETLoader', 'inputs': {'unet_name': 'z_image_turbo_bf16.safetensors', 'weight_dtype': 'default'}},
        '2': {'class_type': 'CLIPLoader', 'inputs': {'clip_name': 'qwen_3_4b.safetensors', 'type': 'qwen_image', 'device': 'default'}},
        '3': {'class_type': 'VAELoader', 'inputs': {'vae_name': 'zimage ae.safetensors'}},
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        r = await c.post('http://127.0.0.1:8188/prompt', json={'prompt': p2})
    print(f'  Status: {r.status_code}')

    # Test 3: With node 11 at the start
    print("\nTest 3: With node 11 at the start")
    p3 = {
        '11': {'class_type': 'easy promptList', 'inputs': {'optional_prompt_list': 't1', 'prompt_1': 't2', 'prompt_2': 't3', 'prompt_3': 't4', 'prompt_4': 't5'}},
        '1': {'class_type': 'UNETLoader', 'inputs': {'unet_name': 'z_image_turbo_bf16.safetensors', 'weight_dtype': 'default'}},
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        r = await c.post('http://127.0.0.1:8188/prompt', json={'prompt': p3})
