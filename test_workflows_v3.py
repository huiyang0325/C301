...
def convert_workflow(workflow, available, object_info=None):
    ...
    for node in workflow['nodes']:
        ...
        # Get the node's input schema if available
        node_input_order = []
        if object_info and ct in object_info:
            input_schema = object_info[ct].get('input', {})
            required = input_schema.get('required', {})
            optional = input_schema.get('optional', {})
            # required and optional are dicts with input names as keys
            node_input_order = list(required.keys()) + list(optional.keys())

        # Build input name -> widget index mapping
        # Only for inputs that have widgets
        input_widget_map = {}
        if node_input_order and wvs:
            # Go through inputs in order and map to widget values
            wvs_idx = 0
            for inp in node.get('inputs', []):
                ...
                if name == '' and ct == 'ReroutePrimitive|pysssss':
                    name = 'value'
                link = inp.get('link')
                # If this input has a link, it doesn't use widget value
                if link is not None and link in link_map:
                    continue
                # If this input has no link, consume widget value
                if wvs_idx < len(wvs):
